var COLOMBIA_DEPTOS_MUNIS = {
  ...
  "Antioquia": ["Medellín","Abejorral","Abriaquí","Alejandría","Amagá","Amalfi","Andes","Angelópolis","Angostura","Anorí","Anzá","Apartadó","Arboletes","Argelia","Armenia","Barbosa","Bello","Betania","Betulia","Belmira","Briceño","Buriticá","Cáceres","Caicedo","Caldas","Campamento","Cañasgordas","Caracolí","Caramanta","Carepa","Carolina del Príncipe","Caucasia","Chigorodó","Cisneros","Ciudad Bolívar","Cocorná","Concepción","Concordia","Copacabana","Dabeiba","Donmatías","Ebéjico","El Bagre","El Carmen de Viboral","El Peñol","El Retiro","El Santuario","Entrerríos","Envigado","Fredonia","Frontino","Giraldo","Girardota","Gómez Plata","Granada","Guadalupe","Guarne","Guatapé","Heliconia","Hispania","Itagüí","Ituango","Jardín","Jericó","La Ceja","La Estrella","La Pintada","La Unión","Liborina","Maceo","Marinilla","Montebello","Mutatá","Nariño","Nechí","Necoclí","Olaya","Peque","Pueblo Rico","Puerto Berrío","Puerto Nare","Puerto Triunfo","Remedios","Rionegro","Sabanalarga","Sabaneta","Salgar","San Andrés de Cuerquia","San Carlos","San Francisco","San Jerónimo","San José de la Montaña","San Juan de Urabá","San Luis","San Pedro de los Milagros","San Pedro de Urabá","San Rafael","San Roque","San Vicente Ferrer","Santa Bárbara","Santa Fe de Antioquia","Santa Rosa de Osos","Santo Domingo","Segovia","Sonsón","Sopetrán","Támesis","Tarazá","Tarso","Titiribí","Toledo","Turbo","Uramita","Urrao","Valdivia","Valparaíso","Vegachí","Venecia","Vigía del Fuerte","Yalí","Yarumal","Yolombó","Yondó","Zaragoza"],
  ...
  "Guainía": ["Inírida","Barrancominas","Cacahual","La Guadalupe","Morichal","Pana Pana","Puerto Colombia","San Felipe"],
...
var _colDeptList = Object.keys(COLOMBIA_DEPTOS_MUNIS).sort(function(a, b) {
  return a.localeCompare(b, 'es');
});

var _colMuniList = [];
_colDeptList.forEach(function(dept) {
  COLOMBIA_DEPTOS_MUNIS[dept].forEach(function(muni) {
    _colMuniList.push({ municipio: muni, departamento: dept });
  });
});
_colMuniList.sort(function(a, b) {
...
function setupGeoAutocomplete(deptInput, muniInput) {
  var deptAC = initAutocomplete(deptInput, {
    minChars: 1,
    items: function() { return _colDeptList; },
    display: function(d) { return '<strong>' + escHtml(d) + '</strong>'; },
    match: function(d, val) { return _normSearch(d).indexOf(_normSearch(val)) >= 0; },
    onSelect: function(d) {
      deptInput.value = d;
      if (muniInput) { muniInput.value = ''; muniInput.focus(); }
    }
  });

  var muniAC = initAutocomplete(muniInput, {
    minChars: 1,
    items: function() {
      var dept = (deptInput.value || '').trim();
      if (dept && COLOMBIA_DEPTOS_MUNIS[dept]) {
        return COLOMBIA_DEPTOS_MUNIS[dept].map(function(m) {
          return { municipio: m, departamento: dept };
        });
      }
      return _colMuniList;
    },
    display: function(m) {
      return '<strong>' + escHtml(m.municipio) + '</strong>' +
             '<div class="ac-sub">' + escHtml(m.departamento) + '</div>';
    },
    match: function(m, val) {
      return _normSearch(m.municipio).indexOf(_normSearch(val)) >= 0;
    },
    onSelect: function(m) {
      muniInput.value = m.municipio;
      deptInput.value = m.departamento;
    }
  });

  return { deptAC: deptAC, muniAC: muniAC };
}
