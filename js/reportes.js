...
async function loadReportes() {
  ...
  try {
    var results = await Promise.all([
      ...
      apiGet('getCambios', { columns: 'id,Empresa,Consecutivo,Estado,Remision_Ingreso,Remision_Salida,Bodega_Ingreso,Bodega_Salida,Observaciones,Producto,Tipo_Linea,Cantidad,Fecha_Ingreso,Fecha_Salida,Fecha_Solicitud' }).catch(function() { return { ok: true, cambios: [] }; }),
...
// LIT_GALON_L, LIT_BIDON_L, _litNum, _litSegLitros y _litParse viven en
// js/shared.js (reutilizados también por js/legalizacion-gastos.js).
...
function _litSetTxt(id, txt) { var el = document.getElementById(id); if (el) el.textContent = txt; }
...
// Colores según signo (verde = neto positivo / a favor, rojo = negativo).
function _litSignColor(n) { return n > 0.0001 ? '#1e8449' : (n < -0.0001 ? '#c0392b' : '#718096'); }
...
function exportLitros() {
  ...
  XLSX.writeFile(wb, 'litros_balance_' + today() + '.xlsx');
