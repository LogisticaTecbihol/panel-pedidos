-- Fix: el rol 'comercial' no podia agregar/editar lineas de sus propios
-- pedidos ("Error: No autorizado" al presionar "Aplicar cambios" o
-- "+Agregar linea"). La RPC editar_pedido_completo es SECURITY DEFINER
-- (se salta RLS) y su chequeo de rol hardcodeado nunca incluyo 'comercial',
-- aunque el frontend (js/auth.js canEdit()) y las politicas RLS de
-- "Pedidos" (add_remisionador_to_all_rls.sql) si le dan ese permiso,
-- restringido a sus propios pedidos (comercial_id = auth.uid() OR
-- creado_por = auth.uid()).
--
-- Se agrega 'comercial' con el mismo criterio de propiedad que el RLS, y
-- ademas se valida que las lineas referenciadas por __row y los
-- p_delete_ids pertenezcan al mismo pedido (Nombre_Empresa+Consecutivo),
-- para que un comercial no pueda tocar filas de otro pedido inyectando un
-- __row ajeno junto con un header propio.
--
-- Base: supabase/migrations/add_fecha_compromiso_editar_pedido_rpc.sql
-- (version viva). Cambia SOLO el bloque de autorizacion; el resto de la
-- logica queda identica.
--
