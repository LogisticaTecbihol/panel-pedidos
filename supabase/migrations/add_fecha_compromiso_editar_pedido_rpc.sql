-- Fase 1 (OTD) — editar_pedido_completo aprende Fecha_Compromiso.
--
-- Base: supabase/migrations/fix_rpc_security_definer_guards.sql:217-342 (version viva).
-- Cambios respecto a esa version, SOLO:
--   * UPDATE de lineas existentes: "Fecha_Compromiso" = COALESCE(p_header->>'Fecha_Compromiso', "Fecha_Compromiso")
--   * INSERT de lineas nuevas: columna "Fecha_Compromiso" con COALESCE(lin->>..., p_header->>..., '')
-- El guard de rol/empresa y el resto de la logica quedan identicos.
--
-- Aplicar con apply_migration del MCP de Supabase.
